...
st.set_page_config(
    ...
    menu_items={
        ...
        'About': "This app was developed by IndigoWizard for environmental monitoring and geospatial analysis."
    }
)
...
def ee_authenticate():
    try:
        if "json_key" in st.secrets:
            ...
            st.info("No service account found. Attempting local authentication...")
            ee.Initialize()
            st.success("Authenticated with Google Earth Engine using local credentials.")
    except Exception as e:
        st.error(f"Failed to authenticate with Google Earth Engine: {str(e)}")
        st.markdown("**Steps to resolve:**\n"
                    "- For local setup, run `earthengine authenticate` in your terminal.\n"
                    "- For cloud deployment, ensure `json_key` is configured in Streamlit Cloud secrets.\n"
                    "- Verify the service account has Earth Engine API permissions.\n"
...
def upload_files_proc(upload_files):
    ...
    for upload_file in upload_files:
        try:
            ...
            features = geojson_data.get('features', []) or [{'geometry': geo} for geo in geojson_data.get('geometries', [])]
            ...
            st.warning(f"Error processing GeoJSON file: {str(e)}")
...
def main():
    ...
    with st.form("input_form"):
        ...
        with c2:
            ...
            cloud_pixel_percentage = st.slider("Cloud pixel rate", min_value=5, max_value=100, step=5, value=75)
            st.info("Upload Area of Interest (GeoJSON):")
            upload_files = st.file_uploader("Create a GeoJSON at [geojson.io](https://geojson.io/)", accept_multiple_files=True)
            geometry_aoi = upload_files_proc(upload_files)
            st.info("Color Palette")
            accessibility = st.selectbox("Accessibility", ["Normal", "Deuteranopia", "Protanopia", "Tritanopia", "Achromatopsia"])
        ...
        # Map generation
        global last_uploaded_centroid
        latitude, longitude = (last_uploaded_centroid[1], last_uploaded_centroid[0]) if last_uploaded_centroid else (36.60, 16.00)
        m = folium.Map(location=[latitude, longitude], tiles=None, zoom_start=11 if last_uploaded_centroid else 5, control_scale=True)
        ...
        # Burn scar
        dNBR_classified = dNBR_classified.gte(4).updateMask(dNBR_classified.neq(0))
        vectors = dNBR_classified.addBands(dNBR_classified).reduceToVectors(
            geometry=geometry_aoi, crs=dNBR_classified.projection(), scale=10,
            geometryType='polygon', eightConnected=False, labelProperty='zone', reducer=ee.Reducer.mean(), bestEffort=True
        )
        ...
        reclassified_dNBR_legend_html = f"""
            <div class="reclassifieddNBR">
                ...
                <ul style="list-style-type: none; padding: 0;">
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[0]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Enhanced Regrowth (High)</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[1]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Enhanced Regrowth (Low)</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[2]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Unburned</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[3]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Low Severity Burns</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[4]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Moderate-Low Severity Burns</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[5]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> Moderate-High Severity Burns</li>
                    <li style="margin: 0.2em 0px;"><span style="background-color: {dNBR_classified_palette[6]}; opacity: 0.75; display: inline-block; width: 15px; height: 15px; border-radius: 50%; margin-right: 5px;"></span> High Severity Burns</li>
                </ul>
            </div>
        """
    ...
    with st.form("report_form"):
        ...
        dNBR_class_areas = [calculate_class_area(masked_dNBR_classified, geometry_aoi, i) / 1e6 for i in range(1, 8)]
        class_names = [
            "Enhanced Regrowth (High)", "Enhanced Regrowth (Low)", "Unburned",
            "Low Severity Burns", "Moderate-Low Severity Burns", "Moderate-High Severity Burns", "High Severity Burns"
        ]
        ...
        if report_form:
            ...
            col2.success(f"**Pre-Fire Date Range:** {str_initial_start_date} to {str_initial_end_date}")
            col2.success(f"**Post-Fire Date Range:** {str_updated_start_date} to {str_updated_end_date}")
                ...
                with elements("nivo_pie_chart"):
                    with mui.Box(sx={"height": 500}):
                        nivo.Pie(
                            ...
                            defs=[
                                {"id": name, "type": "patternDots" if i > 2 else "patternLines",
                                 "color": dNBR_classified_palette[i], "background": f"{dNBR_classified_palette[i]}bf",
                                 "rotation": 105 if i < 2 else -15 if i == 2 else 0,
                                 "lineWidth": 3 if i < 2 else 4 if i == 2 else 0,
                                 "spacing": 10 if i < 3 else 4,
                            ...
                            theme={"tooltip": {"container": {"background": "white", "fontSize": 14, "padding": 2, "border-radius": 4},
                                            "basic": {"background": "#0e1117", "color": "white", "padding": 5}}}
                        )

        # Precipitation Data
        def chirpsCollection(initialDate, updatedDate, aoi):
            return ee.ImageCollection("UCSB-CHG/CHIRPS/DAILY").filterDate(initialDate, updatedDate).filterBounds(aoi).select("precipitation")
        ...
        def full_month_precipitation(initialDate, endDate, aoi):
            ...
            daily_precipitation = raincol.map(
                lambda img: ee.Feature(aoi, {
                    "date": img.date().format("YYYY-MM-dd"),
                    "precipitation": img.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi, scale=30).get("precipitation")
                })
            )
            daily_list = daily_precipitation.getInfo()["features"]
            dates = [entry["properties"]["date"] for entry in daily_list]
            values = [round(entry["properties"]["precipitation"], 2) if entry["properties"]["precipitation"] is not None else None for entry in daily_list]
            return pd.DataFrame({"Date": dates, "Precipitation": values})
        ...
        col5.dataframe(
            rdf, column_config={"Date": "Date", "Precipitation": st.column_config.ProgressColumn("Rainfall (mm)", format="%f mm", min_value=0, max_value=100)},
        ...
        def temperatureCollection(initialDate, updatedDate, aoi):
            return ee.ImageCollection("ECMWF/ERA5_LAND/HOURLY").filterDate(initialDate, updatedDate).filterBounds(aoi).select("temperature_2m")
        ...
        def full_month_temperature(initialDate, endDate, aoi):
            ...
            daily_temperature = tempcol.map(
                lambda img: ee.Feature(aoi, {
                    "date": img.date().format("YYYY-MM-dd"),
                    "temperature": img.reduceRegion(reducer=ee.Reducer.mean(), geometry=aoi, scale=11132).get("temperature_2m")
                })
            )
            daily_temp_list = daily_temperature.getInfo()["features"]
            dates_t = [entry["properties"]["date"] for entry in daily_temp_list]
            values_t = [round(entry["properties"]["temperature"] - 273.15, 2) if entry["properties"]["temperature"] is not None else None for entry in daily_temp_list]
            temp_df = pd.DataFrame({"Date": dates_t, "Temperature": values_t}).groupby("Date", as_index=False).mean()
            return temp_df
        ...
        col5.dataframe(
            temp_df, column_config={"Date": "Date", "Temperature": st.column_config.NumberColumn("Temperature (°C)", format="%.2f °C", min_value=-50, max_value=50)},
            hide_index=True, width=400, height=420
        )

        def temperature_chart(temp_df):
            temp_df["Date"] = pd.to_datetime(temp_df["Date"])
            return alt.Chart(temp_df).mark_bar(color="#e65780").encode(
                x=alt.X("Date:T", axis=alt.Axis(title="Time (Days)")),
                y=alt.Y("Temperature:Q", axis=alt.Axis(title=None)),
                tooltip=["Date:T", "Temperature:Q"]
            ) + alt.Chart(temp_df).mark_line(color="#e63946", point=True).encode(
                x="Date:T", y="Temperature:Q"
            ).properties(title="Daily Temperature (°C)", height=500)
    ...
    st.markdown(
        """
        This app provides tools to analyze wildfire burn severity using NBR/dNBR indices. Considerations:
        - Clouds and water bodies may affect map accuracy.
        - Satellite sensors have limitations in distinguishing surface types.
        - NBR/dNBR values vary with vegetation and land cover.
        """
    )

    st.subheader("Environmental Index: NBR/dNBR")
    st.markdown("The [Normalized Burn Ratio (NBR)](https://www.earthdatascience.org/courses/earth-analytics/multispectral-remote-sensing-modis/normalized-burn-index-dNBR/) uses NIR and SWIR wavelengths:")
    st.latex(r'\text{NBR} = \frac{\text{NIR} - \text{SWIR}}{\text{NIR} + \text{SWIR}}')
    st.latex(r'\text{dNBR} = \text{NBR}_{pre-fire} - \text{NBR}_{post-fire}')

    st.subheader("Data")
    st.markdown("Uses **Sentinel-2 Level-2A** imagery for accurate surface reflectance.")

    st.subheader("Credit")
    st.markdown("Developed by [IndigoWizard](https://github.com/IndigoWizard). Icons by [Flaticon](https://www.flaticon.com/free-icons/wildfire).")
